<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Comments Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used to manage comments in the backend.
    |
    */

    'header' => 'Kommentare',
    'list' => 'Kommentar',
    'my_own' => 'Meine Kommentare',
    'unpublished' => 'Unveröffentlichte Kommentare',
    'state_unpublished' => 'neu (unveröffentlicht)',
    'state_locked' => 'in Bearbeitung (gesperrt)',
    'state_published' => 'erledigt (veröffentlicht)',
    'new' => 'Neuer Kommentar',
    'edit' => 'Kommentar bearbeiten',
    'delete' => 'Kommentar löschen',
    'confirm_delete' => 'Möchten Sie diesen Kommentar wirklich löschen?',
    'created' => 'Kommentar wurde angelegt.',
    'updated' => 'Kommentar wurde bearbeitet.',
    'deleted' => 'Kommentar wurde gelöscht.',
    
    'message' => 'Kommentartext',
    'published' => ':count Kommentare wurden veröffentlicht.',
];
